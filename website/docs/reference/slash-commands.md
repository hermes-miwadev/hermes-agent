...
| `/cc-tasks [limit]` (alias: `/cc_tasks`) | Read-only status listing (default 10, max 50) of recent Claude Code delegations — both `/cc-delegate` and [automatic routing](/user-guide/features/claude-code-auto-routing) — showing each task's ID, worker, workspace, execution state (`running`/`finished`/`interrupted`), and delivery state (`pending`/`delivered`/`failed`). |
...
- `/sethome`, `/restart`, `/approve`, `/deny`, `/topic`, `/platform`, `/commands`, `/cc-delegate`, and `/cc-tasks` are **messaging-only** commands.
